...
export async function searchOmdb(type, query) {
  ...
  const items = await Promise.all(
    res.Search.slice(0, 6).map(async (r, idx) => {
      const detail = idx < 3 ? await fetchJson({ i: r.imdbID, plot: 'short' }).catch(() => null) : null;
      ...
      return {
        ...
        genres: (d.Genre || '').split(',').map((s) => s.trim()).filter(Boolean),
        rating: imdb,
        extraRatings: [
          ratings['Rotten Tomatoes'] ? { source: 'Rotten Tomatoes', value: ratings['Rotten Tomatoes'] } : null,
