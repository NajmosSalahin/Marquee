...
export async function createItem(req, res, next) {
  try {
    const { type, title, posterUrl, overview, releaseYear, genres, externalRating, ratingSource, status, notes, tags, source, externalId } = req.body;
...
const EDITABLE = [
  'title', 'posterUrl', 'overview', 'releaseYear', 'genres', 'externalRating',
  'ratingSource', 'status', 'notes', 'tags', 'source', 'externalId', 'order',
];
...
export async function reorderItems(req, res, next) {
  try {
    ...
    await Promise.all(
      updates.map((u) =>
        WatchlistItem.updateOne({ _id: u.id, userId: req.userId }, { $set: { status: u.status ?? undefined, order: u.order } })
      )
    );
