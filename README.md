...
| Google Books     | Books                                               | Optional free key: Google Cloud Console → enable **Books API** → Credentials → API key. Keyless works but shares a global daily quota (can 429 during peak hours).                                            |
...
TMDB_ACCESS_TOKEN=...
OMDB_API_KEY=...
GOOGLE_BOOKS_API_KEY=...          # optional — keyless books search shares a global quota
...
- **Multi-source search** — every query fans out to two APIs server-side and shows the results side by side with source badges, so a title missing from one source still turns up. One flaky API never blanks the results. Books & manga also support **search by author name** (Google Books `inauthor:` + Open Library `author:`; manga via Jikan people + AniList staff).
- **Authors** — books and manga show their writers on cards, in the list view, and in the detail drawer; editable when adding or later.
