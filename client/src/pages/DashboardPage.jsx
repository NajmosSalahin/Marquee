...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    for (const i of items) {
      ...
      if (i.dateCompleted) {
        ...
        if (d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()) completedThisMonth++;
      }
    }
  ...
  const continuing = useMemo(
    () => items.filter((i) => i.status === 'watching').sort((a, b) => (a.order ?? 0) - (b.order ?? 0)).slice(0, 8),
    [items]
  );
  ...
  return (
    <div className="space-y-10">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Pick up where you left off, or find your next one.</p>
      </div>
      ...
      {itemsQuery.isLoading ? (
        ...
        <>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6" aria-label="Your stats">
            {STATUSES.map((s) => {
          ...
          {stats.completedThisMonth > 0 && (
            <p className="text-sm text-muted">
              <span className="font-mono font-semibold text-accent">{stats.completedThisMonth}</span>{' '}
...
function PosterRow({ items, onPick }) {
  return (
    <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
      {items.map((item) => (
        <motion.div key={item._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
