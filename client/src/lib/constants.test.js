...
import { ACCENTS, SOURCE_LABELS, STATUSES, STATUS_LABELS, TYPES, TYPE_LABELS } from './constants.js'
...
describe('constants', () => {
  ...
  it('has three title types with labels', () => {
    expect(TYPES.map((t) => t.id)).toEqual(['movie', 'tv', 'anime'])
  ...
  it('labels every known source', () => {
    for (const s of ['tmdb', 'omdb', 'jikan', 'anilist', 'manual']) {
