...
export default function ItemDrawer({ item, onClose }) {
  ...
  const [rating, setRating] = useState(
    item?.externalRating != null ? { source: item.ratingSource || 'manual', value: item.externalRating } : null
  );
  ...
  useEffect(() => {
    ...
    setRating(item.externalRating != null ? { source: item.ratingSource || 'manual', value: item.externalRating } : null);
  ...
  const ratingOptions = useMemo(() => {
    const opts = [];
    if (item?.externalRating != null) opts.push({ source: item.ratingSource || 'manual', value: item.externalRating });
    const results = search.data?.results || [];
    for (const r of results) {
      for (const alt of [r, ...(r.alternates || [])]) {
        if (alt.rating != null && alt.rating > 0) opts.push({ source: alt.source, value: alt.rating });
      }
    }
    return opts;
    ...
    <AnimatePresence>
      {item && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
        >
          <button className="absolute inset-0 bg-black/60" onClick={onClose} aria-label="Close details" />
          <motion.aside
            role="dialog"
          ...
          >
            <header className="flex items-center gap-3 border-b border-line px-5 py-4">
              <h2 className="min-w-0 flex-1 truncate font-display text-xl font-semibold text-ink">{item.title}</h2>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5">
              <div className="flex gap-5">
                <div className="w-32 shrink-0 sm:w-40">
                  <Poster src={item.posterUrl} alt={item.title} className="rounded-lg shadow-card" />
                </div>
                <div className="min-w-0 flex-1 space-y-2.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <SourceBadge source={item.source} />
                    <span className="chip">{TYPE_LABELS[item.type]}</span>
                    {item.releaseYear && <span className="font-mono text-sm text-muted">{item.releaseYear}</span>}
                  </div>
                  ...
                  <p className="text-xs text-muted">
                    <CalendarDays className="mr-1 inline h-3.5 w-3.5 align-[-2px]" aria-hidden="true" />
              ...
              {item.overview && <p className="mt-5 text-sm leading-relaxed text-muted">{item.overview}</p>}
              ...
              <div className="mt-6 space-y-5">
                ...
                <div>
                  <label className="label" htmlFor="d-tags">Tags</label>
                  <input
                    id="d-tags"
                    className="input"
                    list="tag-options"
            ...
            <footer className="flex items-center justify-between gap-3 border-t border-line px-5 py-4">
              {confirmingDelete ? (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => remove.mutate()} className="btn bg-red-500/15 text-red-300 hover:bg-red-500/25" disabled={remove.isPending}>
